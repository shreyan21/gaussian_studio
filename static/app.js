...
function setBusy(busy){
 ...
 $('objectOnly').disabled=busy||!['anysplat','sharp'].includes($('engine').value);
...
$('engine').addEventListener('change',()=>{
 const engine=$('engine').value,anysplat=engine==='anysplat',sharp=engine==='sharp',objectCapable=anysplat||sharp;$('qualityField').hidden=engine!=='depth';$('viewBudgetField').hidden=!anysplat;$('objectFocusField').hidden=!objectCapable;$('researchUseField').hidden=!sharp;$('objectOnly').disabled=!objectCapable;$('researchUse').disabled=!sharp;$('depthStrength').disabled=engine!=='depth';
 $('device').querySelector('option[value="cpu"]').disabled=anysplat;
 if(anysplat&&$('device').value==='cpu')$('device').value='auto';
 $('engineDescription').textContent=sharp?'Best one-photo quality. Subject focus enlarges the object and removes the tearing background; rotation stays within the nearby views SHARP can reconstruct.':anysplat?'Jointly predicts cameras and 3D Gaussians from two or more uncalibrated views. NVIDIA CUDA required.':'Predicts depth from one image, then builds a visible Gaussian surface. CPU and CUDA supported.';
});
...
$('createForm').addEventListener('submit',async e=>{
 ...
 if(['anysplat','sharp'].includes(engine)&&$('objectOnly').checked&&health&&!health.models.foreground){showError('Foreground isolation is not installed. Run scripts\\download_models.py --model foreground, then click refresh.');return;}
