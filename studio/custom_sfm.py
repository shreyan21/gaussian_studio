...
def _largest_model(sparse: Path) -> Path:
    ...
    if not models:
        raise RuntimeError(
            "Camera alignment failed. Use at least 6 sharp photos in capture order with 70-85% overlap; "
...
def _reconstruct_cli(
    ...
    return fused, "COLMAP 4.2 CUDA command-line"
...
def _reconstruct_pycolmap(
    ...
    if not maps:
        raise RuntimeError("Camera alignment failed. Capture 20-40 ordered photos with 70-85% overlap.")
    ...
    if not fused.is_file():
        raise RuntimeError("Dense fusion completed without a point cloud. Capture more overlapping, textured views.")
    return fused, f"PyCOLMAP {pycolmap.__version__} CUDA"
...
def dense_cloud_to_gaussians(path: Path, max_gaussians: int = 1_250_000) -> np.ndarray:
    ...
    total = len(vertex)
    if total < 100:
        raise ValueError("Dense reconstruction produced too few points. Add more overlapping views.")
...
def reconstruct(
    ...
    if len(input_paths) < 6:
        raise RuntimeError("Custom reconstruction needs at least 6 overlapping photographs; 20-40 ordered views are recommended.")
    ...
    if executable is not None:
        fused, backend = _reconstruct_cli(executable, images, work, max_side, use_gpu, progress)
    else:
        fused, backend = _reconstruct_pycolmap(images, work, max_side, use_gpu, progress)
    ...
    return gaussians, {
        ...
        "limitation": "Per-scene reconstruction from the supplied photographs. Unseen, reflective, transparent, moving, or poorly matched regions remain incomplete.",
    }
