...
Tests cover photo and streamed-video upload rules, access-token protection, path restrictions, job persistence, video keyframe selection, registration quality gates, gsplat export transforms, dense-fusion recovery, PLY/GSB export, and viewer assets.
...
2. Setup completes checksum verification and doctor reports `custom_engine: true`, `true_3dgs: true`, and `cuda: true`.
...
- **Holes:** surfaces unseen in inputs, moving between frames, or never constrained by registered cameras.
- **CUDA out-of-memory:** close GPU apps, use Quick mode, or reduce photo count while keeping full coverage.
- **Sparse/noisy surface:** verify `representation` is `trained-3dgs`; increase training steps or use Balanced before recapturing.
