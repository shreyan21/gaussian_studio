...
For a flower or single object, leave **Focus central subject** enabled. The app triangulates the subject from recovered camera rays, follows its projected position in every training view, and adjusts each training crop for camera distance so an uneven hand-held orbit does not make the subject tiny in some frames. It keeps the complete COLMAP seed geometry for stable optimization, then filters distant, oversized, low-confidence, and excessively elongated splats during export. The viewer also limits rotation to the useful captured range. Disable focus only when the surrounding environment is intentionally part of the scene.
