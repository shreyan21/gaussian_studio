...
def test_invalid_uploads(client, contents, code):
    files = [("images", ("bad.png", contents, "image/png"))] + views(5)
...
def test_custom_requires_six_views(client):
    assert client.post("/api/jobs", files=views(5)).status_code == 422
    assert client.post("/api/jobs", files=views(6), data={"device": "cpu"}).status_code == 422


def test_upload_names_are_canonicalized(client, monkeypatch):
    monkeypatch.setattr(Jobs, "run", lambda *args: None)
    files = [("images", ("../../unsafe.png", photo(), "image/png"))] + views(5)
    response = client.post("/api/jobs", files=files)
    assert response.status_code == 202
    job = response.json()
    assert job["name"] == "unsafe.png + 5 views"
