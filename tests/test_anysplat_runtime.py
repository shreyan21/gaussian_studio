...
def test_anysplat_foreground_projection_requires_cross_view_consensus():
    masks = np.zeros((2, 5, 5), dtype=bool)
    masks[0, 2, 2] = True
    poses = {
        "extrinsic": torch.eye(4).reshape(1, 1, 4, 4).repeat(1, 2, 1, 1),
    ...
    gaussians = fake_gaussians(1)
    gaussians.means = torch.tensor([[[0.0, 0.0, 1.0]]])

    with pytest.raises(ValueError, match="no valid visible Gaussians"):
        to_viewer_gaussians(gaussians, foreground_mask=masks, camera_poses=poses)
